...
# DESCRIPTION: MongoDB connection management and client initialization
# ============================================================================
...
class MongoDBClientManager:
    ...
    def get_store(self) -> MongoDBStore:
        """
        Get LangGraph store for long-term memory.
        ...
        # Note: The actual vector search index must be created in Atlas
        index_config = VectorIndexConfig(
            dims=config.memory.embedding_dims,
            embed=OpenAIEmbeddings(
                model=config.llm.embedding_model.split(':')[1]
            ) if config.llm.embedding_model.startswith('openai:') else None
        )
